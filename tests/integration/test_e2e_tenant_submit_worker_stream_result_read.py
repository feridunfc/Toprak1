...
import scripts.e2e_tenant_submit_worker_stream_result_read as proof
...
@pytest.mark.asyncio
async def test_e2e_tenant_submit_worker_stream_result_read_passes() -> None:
    artifact = await proof.build_artifact(REDIS_URL)
    ...
    assert artifact["tenant_submit_used"] is True
    assert artifact["tenant_submit_entrypoint"] == "SchedulerLua.enqueue_admitted"
    ...
    assert artifact["result_read_api_used"] is True
    assert artifact["result_readable"] is True
    assert artifact["result_status"] == "done"
...
@pytest.mark.asyncio
async def test_e2e_tenant_submit_worker_stream_result_read_degrades_without_redis() -> None:
    artifact = await proof.build_artifact("redis://localhost:6399/0")

    assert artifact["status"] == "DEGRADED"
    assert artifact["target_claim_supported"] is False
    assert artifact["redis_backend"] == "unavailable"
    assert artifact["tenant_submit_used"] is False
    assert artifact["worker_stream_consume_loop_used"] is False


@pytest.mark.asyncio
async def test_e2e_tenant_submit_worker_stream_result_read_writes_dashboard_artifact(tmp_path, monkeypatch) -> None:
    output = tmp_path / "latest_e2e_tenant_submit_worker_stream_result_read.json"
    monkeypatch.setattr(proof, "OUTPUT_PATH", output)

    artifact = await proof.build_artifact(REDIS_URL)
    proof.write_artifact(artifact)

    assert output.exists()
    data = json.loads(output.read_text(encoding="utf-8"))
    assert data["source"] == "e2e_tenant_submit_worker_stream_result_read"

    if data["status"] == "PASS":
        assert data["target_claim_supported"] is True
        assert data["tenant_submit_used"] is True
        assert data["worker_stream_consume_loop_used"] is True
        assert data["direct_process_message_call_used"] is False
        assert data["result_read_api_used"] is True
        assert data["result_readable"] is True
    else:
