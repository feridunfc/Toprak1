...
OUTPUT_PATH = ARTIFACT_DIR / "latest_e2e_tenant_submit_worker_stream.json"
...
async def build_artifact(redis_url: str | None = None, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return _degraded_artifact(
            f"real Redis unavailable; e2e tenant submit worker stream not proven: {exc}"
        )

    run_id = "run-e2e-tenant-submit-worker-stream-demo"
    tenant_id = "tenant-demo"
    task_id = "task-e2e-tenant-submit-worker-stream-demo"
    shard = 0
    worker_group = "default"
    worker_id = "worker-e2e-tenant-submit-worker-stream-demo"
    payload = payload or {"prompt": "hello e2e tenant submit worker stream"}
...
def main_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate e2e tenant submit worker stream artifact."
    )
