...
def _control_config(
    *,
    ...
    return ControlPlaneConfig(
        instance_id=f"cp-s83-6-{acceptance_id}",
        region="sprint83-6-acceptance",
        stream_shards=1,
...
async def _http_product_scenario(
    ...
    *,
    ...
    worker_id = f"worker-s83-6-{acceptance_id}"
    worker_group = f"group-s83-6-{acceptance_id}"
        ...
        try:
            ...
            submit = await client.post(
                ...
                json={
                    "payload": {
                        "prompt":
                            "Sprint 83.6 HTTP product path",
                    },
                    "agent_type":
                        "sprint83-6-acceptance",
                    "priority": 5,
            ...
            passed = all(
                (
                    ...
                    terminal.get("task_output_status")
                    == "AVAILABLE",
                    terminal.get("task_id") == task_id,
                    terminal.get("task_state") == "done",
                    terminal.get("task_output")
                    == task_output,
                    terminal.get("task_output_issues")
                    == [],
            ...
            return {
                "name":
                    "http_submit_scheduler_worker_terminal_task_output_read",
                ...
                "http_task_state":
                    terminal.get("task_state", ""),
                "http_task_output":
                    terminal.get("task_output"),
                "http_task_output_matches_durable":
                    terminal.get("task_output")
                    == task_output,
                "http_task_output_issue_count":
                    len(
                        terminal.get(
                            "task_output_issues"
                        )
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "sprint": "83.6",
            "source":
                "single_task_output_read_acceptance",
            "status": (
            ...
            "task_output_exposed_in_http_result": passed,
            ...
            "limitations": [
                ...
                "The HTTP output contract is intentionally limited to canonical single-TASK RUNs; multi-task output aggregation remains out of scope.",
