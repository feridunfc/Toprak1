...
OUTPUT_PATH = ARTIFACT_DIR / "latest_worker_stream_consume_loop.json"
...
async def build_artifact(redis_url: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return _degraded_artifact(
            f"real Redis unavailable; worker stream consume loop not proven: {exc}"
        )

    run_id = "run-worker-stream-consume-loop-demo"
    tenant_id = "tenant-demo"
    task_id = "task-worker-stream-consume-loop-demo"
    shard = 0
    worker_group = "default"
    worker_id = "worker-stream-consume-loop-demo"
    payload = {"prompt": "hello worker stream consume loop"}
    ...
    try:
        ...
        artifact = {
            ...
            "dispatch_message_from_lua": bool(commit_sha and run_requested_event_written),
            "run_requested_event_written": run_requested_event_written,
            ...
            "executor_invoked": bool(executor.invocations),
            "fake_executor_used": True,
...
def main_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate worker stream consume loop artifact."
    )
